...
export function parseCliOptions(args: string[]): ParseResult {
  ...
  let renderer = DEFAULT_CLI_OPTIONS.renderer!;
      ...
      switch (arg) {
        ...
        case "--schema": {
          ...
          if (!value || value.startsWith("-")) {
            return { success: false, error: "Missing value for --schema option" };
          }
          schema = value;
          break;
        }
        ...
        case "--model": {
          ...
          const value = args[i];
          if (!value || value.startsWith("-")) {
            return { success: false, error: "Missing value for --model option" };
          }
          model = value;
          break;
        }
        ...
        case "--depth": {
          ...
          if (!depthValue || depthValue.startsWith("-")) {
            return { success: false, error: "Missing value for --depth option" };
          }
          const parsed = parseInt(depthValue, 10);
          if (isNaN(parsed) || parsed < 0) {
            return { success: false, error: `Invalid depth value: ${depthValue}` };
          }
          depth = parsed;
          break;
        }
        ...
        case "--renderer": {
          ...
          if (!rendererValue || rendererValue.startsWith("-")) {
            return { success: false, error: "Missing value for --renderer option" };
          }
        ...
        case "--output": {
          ...
          if (!outputValue || outputValue.startsWith("-")) {
            return { success: false, error: "Missing value for --output option" };
          }
          output = outputValue;
          break;
        }
